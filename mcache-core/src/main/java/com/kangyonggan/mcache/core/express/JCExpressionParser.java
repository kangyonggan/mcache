...
public class JCExpressionParser {
    ...
    public JCTree.JCExpression parse(String express) {
        ...
        String exprSign = StringUtil.EMPTY;

        for (int i = 0; i < express.length(); i++) {
            String str = express.substring(i, i + 1);
            switch (str) {
                case EXPR_PREFIX: {// $
                    if (StringUtil.isNotEmpty(word.toString())) {
                ...
                case EXPR_END: {// }
                    if (StringUtil.in(exprSign, EXPR_BEGIN, EXPR_GET, StringUtil.EMPTY)) {
                ...
                case EXPR_GET: {// .
                    if (!StringUtil.in(exprSign, EXPR_ARR_END, EXPR_LIST_END)) {
                        exprElements.add(new JCExpressionIdent(word.toString()));
                        word = new StringBuilder();
                    }
    ...
    private JCTree.JCExpression convert(List<JCExpressionElement> exprElements) {
            ...
            if (element instanceof JCExpressionLiteral) {
                ...
                for (i = i + 1; i < exprElements.size(); i++) {
                    ...
                    if (nextElement instanceof JCExpressionIdent) {
                        String methodName = "get" + StringUtil.toClassName(((JCExpressionIdent) nextElement).getVariableName());
        ...
        return expression == null ? treeMaker.Literal(StringUtil.EMPTY) : expression;
    }
}
